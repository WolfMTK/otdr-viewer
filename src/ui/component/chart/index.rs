...
const TOOLTIP_W: f64 = 176.0;
const TOOLTIP_H: f64 = 96.0;
...
fn v_per_client_px(rect_height: f64, v_span: f64) -> f64 {
    v_span * VIEW_H / (rect_height * PLOT_H)
}
...
fn movement_v(target: Option<web_sys::EventTarget>, movement_y: f64, v_span: f64) -> Option<f64> {
    let rect = target_rect(target)?;
    Some(movement_y * v_per_client_px(rect.height(), v_span))
}
...
fn render_ab_overlay(
    da: f64,
    db: f64,
    map_x: impl Fn(f64) -> f64,
    on_grab: impl Fn(AbMarker) + Copy + 'static,
) -> impl IntoView {
    let xa = map_x(da);
    let xb = map_x(db);
    let badge = move |x: f64, label: &'static str, marker: AbMarker| {
    ...
    view! {
        ...
        {badge(xa, "A", AbMarker::A)}
        {badge(xb, "B", AbMarker::B)}
    }
}

fn render_tooltip(distances_km: &[f64], levels_db: &[f64], da: f64, db: f64, x: f64, y: f64) -> Option<impl IntoView> {
    ...
    loss.map(|loss| {
        let position_style = format!("left: {x:.0}px; top: {y:.0}px;");
        view! {
            <div class=style::tooltip style=position_style>
...
pub fn Chart(distances_km: Vec<f64>, levels_db: Vec<f64>, events: Vec<SorEvent>) -> impl IntoView {
    ...
    let v_offset = RwSignal::new(0.0_f64);
    Effect::new(move |_| {
        if chart_view.is_fitted() {
            v_offset.set(0.0);
        }
    });
    ...
    let map_y = move |v: f64| PAD_TOP + (v - (v_min + v_offset.get())) / v_span * PLOT_H;
    ...
    let marker_pos = RwSignal::new(ab_endpoints(&events));
    let dragging_marker = RwSignal::new(None::<AbMarker>);
    ...
    let ab_overlay = move || {
        ...
        marker_pos
            .get()
            .map(|(da, db)| render_ab_overlay(da, db, map_x, move |m| dragging_marker.set(Some(m))))
    };
    let tooltip = move || {
        if !markers_visible.get() {
            return None;
        }
        let (da, db) = marker_pos.get()?;
        let (x, y) = mouse_pos.get()?;
        render_tooltip(&distances_km, &levels_db, da, db, x, y)
    };
    ...
    let on_mouse_move = move |ev: web_sys::MouseEvent| {
        if let Some(rect) = target_rect(ev.current_target()) {
            let max_x = (rect.width() - TOOLTIP_W).max(0.0);
            let max_y = (rect.height() - TOOLTIP_H).max(0.0);
            let x = (ev.client_x() as f64 - rect.left() + TOOLTIP_OFFSET).clamp(0.0, max_x);
            let y = (ev.client_y() as f64 - rect.top() + TOOLTIP_OFFSET).clamp(0.0, max_y);
            mouse_pos.set(Some((x, y)));
        }

        if let Some(marker) = dragging_marker.get_untracked() {
            if let Some(km) = pointer_km(ev.current_target(), ev.client_x() as f64, window.get_untracked()) {
                marker_pos.update(|pos| {
                    if let Some((da, db)) = pos {
                        match marker {
                            AbMarker::A => *da = km,
                            AbMarker::B => *db = km,
                        }
        ...
        if let Some(dv) = movement_v(ev.current_target(), ev.movement_y() as f64, v_span) {
            v_offset.update(|o| *o -= dv);
        }
    };
    let stop_drag = move |_: web_sys::MouseEvent| {
        dragging.set(false);
        dragging_marker.set(None);
    };
    let on_mouse_leave = move |ev: web_sys::MouseEvent| {
        stop_drag(ev);
        mouse_pos.set(None);
    };
    ...
    view! {
        ...
        >
                <defs>
                    ...
                    <clipPath id="chart-ab-clip">
                        <rect x=f(PAD_LEFT) y=f(PAD_TOP - 20.0) width=f(PLOT_W) height=f(PLOT_H + 20.0) />
                    </clipPath>
                </defs>
                <rect
                ...
                {y_grid}
                {x_grid}
                {move || ab_overlay().map(|content| view! { <g clip-path="url(#chart-ab-clip)">{content}</g> })}
                {top_ticks}
...
mod tests {
    ...
    use crate::ui::component::chart::index::{
        client_x_to_km, km_per_client_px, v_per_client_px, PAD_LEFT, PAD_RIGHT, VIEW_W,
    };
    ...
    #[rstest]
    fn v_per_client_px_is_proportional_to_v_span() {
        let narrow = v_per_client_px(400.0, 10.0);
        let wide = v_per_client_px(400.0, 100.0);
        approx(wide, narrow * 10.0);
    }

    #[rstest]
    fn v_per_client_px_scales_inversely_with_rect_height() {
        let short_rect = v_per_client_px(200.0, 30.0);
        let tall_rect = v_per_client_px(400.0, 30.0);
        approx(short_rect, tall_rect * 2.0);
    }
}
